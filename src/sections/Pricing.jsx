...
import { onlineClassUrl, packages } from '../data/content'

const offlinePackageLinks = {
  A: '/booking/paket-a',
  B: '/booking/paket-b',
  C: '/booking/paket-c',
  D: '/booking/paket-d',
...
export default function Pricing({ lang, t }) {
  return <section id="paket" className="py-24 sm:py-36"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    ...
    <div className="mt-14 overflow-hidden rounded-3xl border border-line">
      {packages[lang].map((item, index) => {
        const bookingPath = offlinePackageLinks[item.code]
        const actionClass = 'group mt-6 inline-flex items-center gap-2 text-xs font-semibold text-ink hover:text-teal'

        return <article key={item.code} className={`relative grid gap-7 border-b border-line p-6 last:border-0 sm:p-8 lg:grid-cols-[140px_1fr_230px] lg:items-start ${item.featured ? 'bg-mint' : 'bg-white'}`}>
        ...
        <div className="flex flex-col border-t border-line pt-5 lg:items-end lg:border-l lg:border-t-0 lg:pl-7 lg:pt-0">
          ...
          {bookingPath
            ? <Link to={bookingPath} className={actionClass}>{t.packages.choose}<ArrowIcon className="size-4 transition group-hover:translate-x-1" /></Link>
            : <a href={onlineClassUrl} target="_blank" rel="noreferrer" className={actionClass}>{t.packages.choose}<ArrowIcon className="size-4 transition group-hover:translate-x-1" /></a>}
        </div>
      </article>
      })}
    </div>
